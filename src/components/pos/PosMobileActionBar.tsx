...
import { Bell, ReceiptText, Store, Lock } from 'lucide-react';
    ...
    <div className="lg:hidden fixed bottom-0 inset-x-0 h-[60px] bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-around z-40 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.05)] pb-safe">
      ...
      {onLockScreen && (
        <button
          ...
          className="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors"
        >
          <Lock className="w-5 h-5 mb-1" />
          <span className="text-[10px] font-medium">Kunci</span>
        </button>
      )}
    </div>
  );
